...
class Gnet:
    ...
    def __init__(self,params,z,setbn=False):
        self.params = params
        self.weights = {}
        self.bnops = []
        ...
        om_ = tf.Variable(tf.zeros([f1]))
        ov_ = tf.Variable(tf.zeros([f1]))
        self.weights['bnm_c%d_w'%lnum] = om_
        self.weights['bnv_c%d_w'%lnum] = ov_

        if setbn:
            om,ov = tf.nn.moments(out,[0,1,2])
            self.bnops.append(tf.assign(om_,om).op)
            self.bnops.append(tf.assign(ov_,ov).op)
        else:
            om = om_
            ov = ov_
